...
class DBWNode(object):
        ...
        while not rospy.is_shutdown():
            if not None in (self.current_vel,self.linear_vel,self.angular_vel):
                self.throttle,self.brake,self.steering = self.controller.control(self.current_vel,self.dbw_enabled,
                    ...
                    #way_y = self.base_waypoints.waypoints[1].pose.pose.position.y
                    #car_y = self.pose.position.y
                    #rospy.loginfo("car_y is %s",car_y)
                    #rospy.loginfo("way_y is %s",way_y)
                    #rospy.loginfo("difference is %s",car_y - way_y)
                    #if abs(way_y - car_y) > 0.75:
			#self.brake_applied = 1
                        #self.throttle = self.throttle/2.
                        #self.brake = self.brake * 2. + 0.1
                if self.dbw_enabled:
    ...
    def waypoints_cb(self, waypoints):
        # TODO: Implement
        #rospy.loginfo("waypoints cb called")
    ...
    def twist_cb(self,msg):
        ...
        #rospy.loginfo('dbw-node -->  linear velocity receuved is %s', self.linear_vel)
