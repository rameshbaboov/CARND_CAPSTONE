...
from styx_msgs.msg import TrafficLightArray, TrafficLight, Lane, TLStatus
...
class TLDetector(object):
    ...
    def __init__(self):
        ...
        tl_class_model = self.config['tl']['tl_classification_model']
        self.light_classifier = TLClassifier()
        model = load_model(tl_class_model)
        ...
        tl_detect_model = self.config['tl']['tl_detection_model']
        self.detector_model = load_model(tl_detect_model,custom_objects = custom_object)
        ...
        self.sub1 = rospy.Subscriber('/base_waypoints', Lane, self.waypoints_cb)
        ...
        self.upcoming_red_light_pub = rospy.Publisher('/traffic_waypoint', TLStatus, queue_size=1)
...
################################################################################################
    ...
    def waypoints_cb(self, msg):
            ...
            for i in range(len(self.waypoints.waypoints)):
                self.waypoints.waypoints[i].pose.header.frame_id = self.waypoints.header.frame_id
    ...
    def publish_upcoming_red_light(self,light_wp,state):
        ...
        if self.state != state:
            ...
            light_wp = light_wp if (state == TrafficLight.RED or state == TrafficLight.YELLOW or state == TrafficLight.GREEN) else -1   
            self.last_wp = light_wp
            rospy.loginfo("Light Wp is %s and state is %s", light_wp, self.state)
            self.upcoming_red_light_pub.publish(tl_status_msg)
        else:  
            tl_status_msg = TLStatus()
            tl_status_msg.header.stamp = rospy.Time.now()
...
################################################################################################
    ...
    def process_traffic_lights(self):
        ...
        """
        if (self.waypoints is None or self.pose is None or len(self.stoplines_wp) == 0 ):
            rospy.loginfo("[TL_DETECTOR] No TL is detected. None")
            return -1, TrafficLight.UNKNOWN
            # find the closest visible traffic light (if one exists)
            
        light = self.get_closest_visible_traffic_light(self.pose.pose)
        if light is None:
            rospy.loginfo("[TL_DETECTOR] No TL is detected. None")
            return -1, TrafficLight.UNKNOWN
    ...
    def calculate_traffic_light_waypoints(self):
        ...
        """
        if self.waypoints is not None and self.lights is not None and len(self.light_wp) == 0:
            for i in range(len(self.lights)):
                ...
                self.light_wp.append(self.get_closest_waypoint(self.lights[i].pose.pose))
    ...
    def get_closest_visible_traffic_light(self, pose):
        ...
        num_lights = len(self.light_wp)
        dist_min = sys.maxsize
        light_min = None

        for light in range(num_lights):
            dist = self.distance2(pose, self.waypoints.waypoints[self.light_wp[light]].pose.pose)

            if dist < dist_min:
                dist_min = dist
                light_min = light

        transformed_waypoint = self.transform_to_car_frame(self.waypoints.waypoints[self.light_wp[light_min]].pose)
        ...
        #dist2 = self.distance2(pose, self.waypoints.waypoints[self.light_wp[light_min]].pose.pose)

        #if dist2 > (TRAFFIC_LIGHT_DIST**2):
            #return None
